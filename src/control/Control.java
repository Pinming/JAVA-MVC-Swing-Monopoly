...
//		int movePixel = 1;
//
//        if (p.getX() == 9 * 60 && p.getY() >= 0 && p.getY() < 60) {
//			// [0,9]
//			// ↓
//			p.setY(p.getY() + movePixel);
...
//	}
	private void move01() {
		...
		// 单位移动像素
		int movePixel = 1;
		if (p.getX() < 13 * 60 && p.getY() == 0) {
			p.setX(p.getX() + movePixel);
		} else if (p.getX() == 13 *60 && p.getY() < 8 * 60){
			p.setY(p.getY() + movePixel);
		} else if (p.getX() > 0 && p.getY() == 8 * 60){
			p.setX(p.getX() - movePixel);
		} else if (p.getX() == 0 && p.getY() > 0){
			p.setY(p.getY() - movePixel);
		}
...
//	}
	...
	private void stopInPoint(Building b, PlayerModel player) {
		player.setCash(((Point) b).getPoint() + player.getCash());
		this.textTip.showTextTip(player, player.getName() + " 获得 "
				+ ((Point) b).getPoint() + "金币.", 3);
		new Thread(new MyThread(run, 1)).start();
	}
	...
	private void stopInOrigin(Building b, PlayerModel player) {
		this.textTip.showTextTip(player, player.getName() + " 在起点停留，奖励 "
				+ ((Origin) b).getReward() + "金币."+this.run.getNowPlayer().getX()+"xxxx"+this.run.getNowPlayer().getY()+"xx", 3);
...
//				System.out.println(player.getCards().get(i).getcName());
	...
	private void stopInLottery(Building b, PlayerModel player) {
		int random = (int) (Math.random() * ((Lottery) b).getImgageEvents().length);
		switch (random) {
		case 0:
			player.setCash(player.getCash() + 1000);
			break;
		...
		this.events.showImg(((Lottery) b).getImgageEvents()[random], 3, new Point(
				420, 160, 0));
		new Thread(new MyThread(run, 1)).start();
	}
