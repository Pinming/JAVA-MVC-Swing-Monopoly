package BBZillionaire.ui.config;
...
import BBZillionaire.ui.config.Photo;
...
import BBZillionaire.control.Control;
import BBZillionaire.control.GameRunning;
...
public class FrameConfig extends JFrame {
	...
	private Component createGameSelectPanel() {
		...
		final String[] cash_ = { "5000", "7500", "10000", "12500", "15000",
				"17500" };
...
//	}
	...
	private void addPlayer01Config(int x, int y, JPanel jp) {
		...
		jlPlayer01Selected.setVisible(false);
		jbnPlayerNameField01.setEditable(false);//设置名字文本框不可编辑
		// 创建左按钮
		...
		jbnPlayer01.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				if ((chooses[0] != selected[1])) {
					...
					JOptionPane.showMessageDialog(null, "人物不可重复");
				}
			}
		});
		jp.add(jbnPlayer01);
...
//		});
	...
	private void addPlayer02Config(int x, int y, JPanel jp) {
		...
		jbnPlayerNameField02.setText(pName[chooses[1]]);
		jlPlayer02Choose.setBounds(x + 8, y, 128, 128);
		// 创建人物图像已选择label
		jlPlayer02Selected.setBounds(x + 8, y, 128, 128);
		jlPlayer02Selected.setVisible(false);
		jbnPlayerNameField02.setEditable(false);//设置名字文本框不可编辑
		// 创建左按钮
...
//				} 
			...
			private void dealPlayers(Control control) {
				...
				// 传入名字
				tempPlayer.get(0).setName(jbnPlayerNameField01.getText());
				tempPlayer.get(1).setName(jbnPlayerNameField02.getText());
				// 传入使用角色编号
